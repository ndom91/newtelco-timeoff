...
import { Avatar, Tag, Panel, Button } from 'rsuite'
...
const Comment = props => {
  ...
  if (props.data.userDetails) {
    initials =
      props.data.userDetails.fname.substr(0, 1) +
      props.data.userDetails.lname.substr(0, 1)
  }
  return (
    <Panel className='comment-panel' bordered>
      <div className='comment-header'>
        ...
        {props.data.datetime ? (
          <Tag color='green' className='comment-user-datetag'>
            {isValid(new Date(props.data.datetime)) &&
              format(new Date(props.data.datetime), 'dd.MM.yyyy HH:mm')}
          </Tag>
        ) : (
          ''
        )}
      </div>
      <div className='comment-body'>{props.data.body}</div>
