  ...
  users.forEach(async user => {
    ...
    if (attemptCount === userCount) {
      if (userSuccessCount === userCount) {
        res.status(200).json({ status: 200, text: `Successfully added ${userCount} users` })
      } else {
        res.status(200).json({ status: 500, text: `Error adding ${userCount - userSuccessCount} users`, error: lastError })
      }
    }
  })
}
