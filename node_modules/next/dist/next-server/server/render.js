...
function renderDocument(Document, { dataManagerData, props, docProps, pathname, query, buildId, canonicalBase, assetPrefix, runtimeConfig, nextExport, autoExport, skeleton, dynamicImportsIds, dangerousAsPath, hasCssMode, err, dev, ampPath, ampState, inAmpMode, hybridAmp, staticMarkup, devFiles, files, polyfillFiles, dynamicImports, htmlProps, bodyTags, headTags, }) {
    return ('<!DOCTYPE html>' +
...
async function renderToHTML(req, res, pathname, query, renderOpts) {
    ...
    const callMiddleware = async (method, args, props = false) => {
        ...
        if (Document[`${method}Middleware`]) {
            const curResults = await Document[`${method}Middleware`](...args);
            if (props) {
